...
// Translate backend error messages based on Accept-Language header
function translateBackendError(err, acceptLang) {
  if (!err || typeof err !== "string") return err;
  const wantsZh = acceptLang && acceptLang.includes("zh");
  if (!wantsZh) return err;

  // Already has Chinese, return as-is
  if (/[一-鿿]/.test(err)) return err;

  // Translate known English error hints
  if (err.includes("incomplete or corrupted")) {
    return err.replace("is present but appears incomplete or corrupted. Delete it from Local Models and download/import it again.",
      "已存在但似乎损坏或不完整。请从本地模型中删除并重新下载/导入。");
  }
  if (err.includes("could not be loaded as a single checkpoint")) {
    return err.replace("could not be loaded as a single checkpoint. Some GGUF files are only the diffusion part of a larger workflow and need separate VAE/text encoder files. Try a recommended SD 1.5/SDXL model, or re-download/import the file if this is meant to be a single-file SD/SDXL GGUF.",
      "无法作为单个检查点加载。部分 GGUF 文件仅包含扩散部分，需要单独的 VAE/文本编码器文件。请尝试推荐使用的 SD 1.5/SDXL 模型，或重新下载/导入文件（如果它应该是单个 SD/SDXL GGUF 文件）。");
  }
  if (err.includes("The backend could not create the model context")) {
    return err.replace("The backend could not create the model context. Common causes are a corrupt/incomplete model file, unsupported checkpoint type, or not enough free RAM/VRAM. Delete and re-download the model, then try CPU or Vulkan mode at 512x512.",
      "后端无法创建模型上下文。可能原因：模型文件损坏/不完整、不支持的检查点类型，或可用 RAM/显存不足。请删除并重新下载模型，或尝试 CPU/Vulkan 模式（512x512）。");
  }
  return err;
}
...
function pollNvidiaVram(force = false) {
  ...
  const now = Date.now();
  // If not forced, skip if it has been polled too recently
  if (!force) {
    if (now - lastVramPollTime < 3000) {
      return;
    }
  }
  ...
  const cmd = `${nvidiaSmiCmd} --query-gpu=name,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits`;
  exec(
    cmd,
    ...
    (error, stdout) => {
      ...
      const [name, usedMb, totalMb, gpuUtil] = firstLine.split(",").map(part => part.trim());
      cachedVramInfo = {
        ...
        gpu_usage: parseFloat(gpuUtil) || 0,
      };
    }
  );
}

// Start background VRAM polling (runs every 3 seconds, active even during generation)
setInterval(() => pollNvidiaVram(false), 3000);
pollNvidiaVram(true);
...
function backendAccepts(binaryPath, backendName) {
  ...
  try {
    // 快速探测：用不存在的模型让进程快速退出，检查输出中是否有后端设备信息
    ...
    const output = `${stdout}\n${stderr}`;
    console.log(`[probe] ${backendName}: status=${result.status}, signal=${result.signal}, stdout=${stdout.length}B, stderr=${stderr.length}B`);
    // 明确的后端不支持标志
    if (output.includes("backend config failed") || output.includes(`backend '${backendName}' was not found`)) {
      console.log(`[probe] ${backendName}: REJECTED (backend not found)`);
      return false;
    }
    // 有任何输出 = 后端可启动（Vulkan 设备信息、CUDA 信息、文件不存在错误等）
    // 无输出 + 非零退出码 = 缺少运行时 DLL（如 CUDA DLL 缺失导致崩溃）
    if (output.trim().length > 0) {
      console.log(`[probe] ${backendName}: ACCEPTED (has output)`);
      return true;
    }
    if (result.status !== 0) {
      console.log(`[probe] ${backendName}: REJECTED (no output, crashed)`);
      return false;
    }
    console.log(`[probe] ${backendName}: ACCEPTED (exit 0)`);
    return true;
  } catch (e) {
    console.log(`[probe] ${backendName}: REJECTED (exception: ${e.message})`);
    return false;
  }
}
...
function selectBackendPath(useGpu, backendType = "auto") {
  ...
  if (osPlatform === "win32" && resolvedType === "vulkan" && fs.existsSync(BACKEND_PATHS.vulkan)) {
    return BACKEND_PATHS.vulkan;
  }
  if (osPlatform === "linux" && resolvedType === "vulkan" && fs.existsSync(BACKEND_PATHS.linux)) {
    return BACKEND_PATHS.linux;
  }
  // CPU mode: use the default binary (which supports --backend cpu)
  if (osPlatform === "win32" && fs.existsSync(BACKEND_PATHS.vulkan)) {
    return BACKEND_PATHS.vulkan;
  }
  if (osPlatform === "win32" && fs.existsSync(BACKEND_PATHS.cuda)) {
    return BACKEND_PATHS.cuda;
  }
  return BACKEND_PATH;
}
...
function describeBackendError(rawError, modelPath, wantsChinese = false) {
  const raw = String(rawError || "").trim();
  const filename = path.basename(modelPath || "");
  const lower = filename.toLowerCase();
  const modelName = filename || "所选模型";

  if (raw.includes("read tensor data failed") || raw.includes("load tensors from file failed")) {
    if (wantsChinese) return `${raw}\n\n${modelName} 已存在但似乎损坏或不完整。请从本地模型中删除并重新下载/导入。`;
  ...
  if (lower.endsWith(".gguf")) {
    if (wantsChinese) return `${raw}\n\n${modelName} 无法作为单个检查点加载。部分 GGUF 文件仅包含扩散部分，需要单独的 VAE/文本编码器文件。请尝试推荐使用的 SD 1.5/SDXL 模型，或重新下载/导入文件（如果它应该是单个 SD/SDXL GGUF 文件）。`;
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/api/backend-status" && req.method === "GET") {
    return json(res, 200, { 
      ...
      error: translateBackendError(backendError, req.headers["accept-language"]),
  ...
  if (req.url.startsWith("/api/backend-options") && req.method === "GET") {
    const urlObj = new URL(req.url, `http://${req.headers.host || "localhost"}`);
    if (urlObj.searchParams.get("refresh") === "true") {
  ...
  if (req.url === "/api/restart-backend" && req.method === "POST") {
    ...
    try {
      ...
      return json(res, 500, { ok: false, error: translateBackendError(backendError, req.headers["accept-language"]), port: PORT_BACKEND });
    }
  }
  ...
  // ── Proxy /v1/ and /sdapi/ to backend (sd-vulkan.exe on PORT_BACKEND) ─────
  // 注意：不要在 backendReady 缺失时回退到 static frontend —— 否则会返回 HTML
  // 让 Ktor 误以为是 JSON 失败。直接代理，由 sd-vulkan 返回 4xx 错误（清晰可读）。
  if (req.url.startsWith("/v1/") || req.url.startsWith("/sdapi/")) {
    if (!backendReady) {
      return json(res, 503, {
        error: "Backend not ready. Please select a model and click 'Load model' in the app's Models page first.",
        code: "BACKEND_NOT_READY",
        backendReady: false,
        port: PORT_BACKEND
      });
    }
    const backendUrl = `http://127.0.0.1:${PORT_BACKEND}${req.url}`;
    const proxyReq = http.request(backendUrl, {
      method: req.method,
      headers: { ...req.headers, host: `127.0.0.1:${PORT_BACKEND}` },
    }, (proxyRes) => {
    ...
    proxyReq.on("error", (err) => {
      ...
      res.end(JSON.stringify({ error: `Backend not reachable: ${err.message}` }));
    });
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    // HTML files should never be cached (they reference hashed JS/CSS)
    if (ext === ".html") {
      headers["Cache-Control"] = "no-cache, no-store, must-revalidate";
      headers["Pragma"] = "no-cache";
    } else if (ext === ".js" || ext === ".css") {
      // Hashed assets can be cached; unhashed ones should not
